...
const ConsultationScheduleDetail = () => {
  ...
  return (
    <Card className="rounded-xl shadow-md bg-white border border-blue-100">
      <CardHeader className="bg-[#E3F2FD] rounded-t-xl border-b border-blue-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 py-4 px-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-2xl font-bold text-blue-800">Chi tiết lịch tư vấn</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-900">{data.parentName}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="font-semibold text-gray-700 min-w-[120px]">Thời gian:</span>
              <span className="text-gray-900">{data.scheduleTime ? new Date(data.scheduleTime).toLocaleString() : ''}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="font-semibold text-gray-700 min-w-[120px]">Lý do:</span>
              <span className="text-gray-900">{data.reason}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="font-semibold text-gray-700 min-w-[120px]">Trạng thái:</span>
              <span className={`px-3 py-1 rounded-full text-xs font-semibold shadow-sm ${statusMap[data.status]?.color || 'bg-gray-200 text-gray-800'}`}>{statusMap[data.status]?.label || data.status}</span>
            </div>
          </div>
        ) : null}
      </CardContent>
